...
if (fs) {
  fs.watch($tw.boot.wikiTiddlersPath, function (eventType, filename) {
    ...
    if (filename) {
      ...
      if(filename.slice(0,10) !== "Draft of '") {
      ...
      } else if (filename.slice(0,10) === "Draft of '") {
        // The new file is a draft tiddler so we need to list the tiddler as
        // being edited in all the wikis
        // We need to figure out how to determine when the editing ends
        console.log('Draft tiddler')
        if (fs.existsSync(`${$tw.boot.wikiTiddlersPath}/${filename}`)) {
          if ($tw.GateKeeper.EditingTiddlers[tiddlerObject.tiddlers[0].title]) {
            // Tiddler already marked as being edited, do nothing.
            console.log('edited');
          } else {
            // The draft tiddler was just created, add to the currently edited
            // list for all connected tiddlers.
            console.log('created');
            // Take the list from $tw.GateKeeper.EditingTiddlers, create a
            // tiddler from it and push that tiddler to all wikis.
            // Another part of the plugin will change the edit button so that it
            // will only appear on tiddlers that aren't listed in that tiddler.
            $tw.GateKeeper.EditingTiddlers[tiddlerObject.tiddlers[0]['draft.of']] = true;
          ...
          console.log('deleted');
          // Remove the tiddler title from the $tw.GateKeeper.EditingTiddlers
          // list and update all the wikis
          var tiddlerTitle = filename.slice(10, -5);
          if ($tw.GateKeeper.EditingTiddlers[tiddlerTitle]) {
            console.log($tw.GateKeeper.EditingTiddlers)
            delete $tw.GateKeeper.EditingTiddlers[tiddlerTitle];
            var tiddlerFields = {title: "$:/GateKeeper/EditingTiddlers", list: $tw.utils.stringifyList(Object.keys($tw.GateKeeper.EditingTiddlers))}
            console.log($tw.GateKeeper.EditingTiddlers)
            $tw.connections.forEach(function (connection, index, connections) {
              connection.socket.send(JSON.stringify({type: 'makeTiddler', fields: tiddlerFields}));
